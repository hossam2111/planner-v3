...
let modalStr = \`onclick="document.getElementById('wirdWelcomeModal').style.display='none'; tab('wird', document.querySelectorAll('.tab')[1])"\`;
let modalStrNew = \`onclick="document.getElementById('wirdWelcomeModal').style.display='none'; openQuranReader()"\`;

html = html.replace(modalStr, modalStrNew);
// In case they click outside or it already has different spacing
html = html.replace(/onclick="document\.getElementById\('wirdWelcomeModal'\)\.style\.display='none'; tab\('wird', document\.querySelectorAll\('\.tab'\)\[1\]\)"/g, modalStrNew);
